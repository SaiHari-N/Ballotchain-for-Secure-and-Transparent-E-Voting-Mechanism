...
$aq=mysqli_query($connect,"select * from vt_status where uname='$user'");
...
//$qgg="select * from vt_voter where aadhar='$aadhar' && user='$user'";
	$q1=mysqli_query($connect,"select * from vt_voter where aadhar='$aadhar' && user='$user'");
...
{
$vdate=date("d-m-Y");
	$q1=mysqli_query($connect,"select * from vt_status where uname='$user' && vdate='$vdate'");
	$n1=mysqli_num_rows($q1);
	
	if($n1>0)
	{
$mq=mysqli_query($connect,"select max(id) from vt_temp where user='$user'");
$mr=mysqli_fetch_array($mq);
$id=$mr['max(id)']+1;
$_SESSION['sess_id']=$id;
$qry=mysqli_query($connect,"insert into vt_temp(id,vid,status,user) values($id,'0','0','$user')");
