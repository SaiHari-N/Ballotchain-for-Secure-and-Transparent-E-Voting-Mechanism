...
<body>
	...
	$aq1=mysqli_query($connect,"select * from vt_status where uname='$user'");
			...
			if($est=="3")
			{
					...
					$q2=mysqli_query($connect,"select distinct(caption) from vt_candidate where user='$user'");
						while($r2=mysqli_fetch_array($q2))
						{
						$cap[]=$r2['caption'];
						}
					$q3=mysqli_query($connect,"select * from vt_division where user='$user' order by id");
			...
			//print_r($acnt);
			///////
			mysqli_query($connect,"delete from vt_temp where user='$user'");
			...
			$id=$mr['max(id)']+1;
					if($page=="3")
					{
					$qq1=mysqli_query($connect,"select * from vt_division where caption='$cap[$j]' && user='$user'");
					$seats=mysqli_num_rows($qq1);
					}
					else if($page=="2")
					{
					$qq1=mysqli_query($connect,"select * from vt_division where caption2='$cap[$j]' && user='$user'");
					$seats=mysqli_num_rows($qq1);
					}
					else
					{
					$qq1=mysqli_query($connect,"select * from vt_division where caption1='$cap[$j]' && user='$user'");
					$seats=mysqli_num_rows($qq1);
					}
			
			$qry=mysqli_query($connect,"insert into vt_temp(id,caption,vcount,seats,user) values($id,'$cap[$j]','$acnt[$j]','$seats','$user')");
			}
			////////
			$q4=mysqli_query($connect,"select * from vt_temp where user='$user' order by seats desc");
		...
		?>
                ...
                <div class="row">
					...
					<div class="board">
						...
						<div class="icon">
							<?php
							$q5=mysqli_query($connect,"select * from vt_candidate where caption='$cap1[$j]' && user='$user'");
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                <div class="list-group">
									<?php
									$q6=mysqli_query($connect,"select * from vt_temp where user='$user' order by seats desc");
									while($r6=mysqli_fetch_array($q6))
									{
									$qq31=mysqli_query($connect,"select * from vt_candidate where caption='".$r6['caption']."' && user='$user'");	
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        <tbody>
											<?php
											$i=0;
											$qq2=mysqli_query($connect,"select * from vt_division where user='$user' order by district");
											while($rr2=mysqli_fetch_array($qq2))
											{
											$i++;
												$qq3=mysqli_query($connect,"select * from vt_candidate where caption='".$rr2['caption']."' && user='$user'");	
 ...
 window.location.href="result.php?page=3";
	 ...
	 ?>           
		...
		?>
				<footer><p>Voting System <a href="http://webthemez.com"></a></p>
