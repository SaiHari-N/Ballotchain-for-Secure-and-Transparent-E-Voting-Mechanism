...
$q1=mysqli_query($connect,"select * from vt_voter where id=$id && user='$user'");
...
$qq=mysqli_query($connect,"select * from vt_candidate where division='$divi' && user='$user'");

if($vote=="yes")
{
mysqli_query($connect,"update vt_candidate set vcount=vcount+1 where id=$cid && user='$user'");
mysqli_query($connect,"update vt_voter set vote_st=1,cid=$cid where id=$id && user='$user'");

$q2=mysqli_query($connect,"select * from vt_candidate where id=$cid && user='$user'");
...
/////////////////////////////////////////////
	 $q3=mysqli_query($connect,"select * from vt_blockchain where user='$user' order by id desc limit 0,1");
	...
	mysqli_query($connect,"insert into vt_blockchain(id,block_id,pre_hash,hash_value,sdata,status,user) values($id2,'$id','$pre_hash','$hash','$data','vote','$user')");
	...
	mysqli_query($connect,"update vt_voter set status=0,vote_st=1 where id='$id'");
	
	
	$mq3=mysqli_query($connect,"select max(id) from vt_vote_reg");
	$mr3=mysqli_fetch_array($mq3);
	$id3=$mr3['max(id)']+1;
	mysqli_query($connect,"insert into vt_vote_reg(id,vid,cid,caption,user) values($id3,$id,$cid,'$caption','$user')");
